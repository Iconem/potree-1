...
float ztransform(vec2 fragPos){
    ...
    float z_n = 2.0 * texture2D(uEDLDepth, fragPos).r - 1.0; //z_n between -1 and 1
...
void main(){
	...
	// Compute normal pass from depth buffer
	gl_FragColor = vec4(getNormalFromDepth(vUv), opacity);
